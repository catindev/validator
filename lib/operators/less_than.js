...
module.exports = function less_than(ctx, rule) {
  ...
  if (!t) {
    return [outcome("EXCEPTION", "RULE_INVALID", "less_than requires comparable values (number-number or date-date)", {
      field,
      rule_id: ctx.ruleId,
  ...
  return [outcome(rule.level || "ERROR", rule.code || "NOT_LESS_THAN", rule.message || "Value must be less than", {
    field,
    rule_id: ctx.ruleId,
    details: { maxExclusive: b.repr || rule.value, actual: a.repr || v }
  })];
};
