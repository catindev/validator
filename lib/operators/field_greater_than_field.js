...
module.exports = function field_greater_than_field(ctx, rule) {
  ...
  if (!t) {
    return [outcome("EXCEPTION", "RULE_INVALID", "field_greater_than_field requires comparable values (number-number or date-date)", {
      field,
      rule_id: ctx.ruleId,
  ...
  return [outcome(rule.level || "ERROR", rule.code || "FIELD_NOT_GREATER_THAN_FIELD", rule.message || "Field must be greater than other field", {
    field,
    rule_id: ctx.ruleId,
